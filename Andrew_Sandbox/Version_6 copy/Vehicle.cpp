...
//#undef DEBUG_ROAD_RULES // Turn off DEBUG_ROAD_RULES
...
// The boolean used to determine if we can send
bool can_transmit = false;
pthread_mutex_t tx_mutex = PTHREAD_MUTEX_INITIALIZER;

// The boolean used to determine if we can step through our main program
// Reason: Need to update location based on speed on some scheduled interval
bool can_step = false;
pthread_mutex_t main_step_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void *timer_main_loop(void *time_ms)
{
	while (1)
	{
		// Sleep for time_ms
		...
		// Lock the mutex
		pthread_mutex_lock(&main_step_mutex);
		// Set global boolean to true
		can_step = true;
		// Unlock the mutex
		pthread_mutex_unlock(&main_step_mutex);
	}
}
...
int main(int argc, const char * argv[]) {
	...
	pthread_t server_thread, timer_thread, timer_main_loop_thread;
	...
	// Create thread to wait to send status packets
	int status_send_micro_s = 100000; // Set the timer to 100 milliseconds
//	int status_send_micro_s = 3000000; // Set the timer to 3 seconds
	//	int micro_s = 1000000; // Set the timer to 1 second
	rc = pthread_create(&timer_thread, NULL, timer, &status_send_micro_s);
	// check for creation errors
	if (rc)
	{
		cout << "Error: unable to create thread," << rc << endl;
	...
	int main_loop_timer_micro_s = 1000; // Set the timer to 1 milliseconds
//	int main_loop_timer_micro_s = 1000000; // Set the timer to 1 second
	rc = pthread_create(&timer_main_loop_thread, NULL, timer_main_loop, &main_loop_timer_micro_s);
	// check for creation errors
	if (rc)
	{
		cout << "Error: unable to create thread," << rc << endl;
...
//			cout << "Main Step.\n";
				...
				if (incoming_source_address != my_address)
				{
					...
					if (packet_in.packet_type == LOCATION_PACKET)
					{
						...
						if (packet_in.x_position <= nodes[my_node_num].node_x_coordinate + DANGER_CLOSE // Car ahead is less than DANGER_CLOSE
							...
							&& packet_in.x_speed <= speed_meter_per_sec // <= so this will keep being executed in LEFT LANE clear
			...
			// Update our new X location
			set_new_location(nodes[my_node_num].node_x_coordinate, speed_meter_per_sec, main_loop_timer_micro_s);
