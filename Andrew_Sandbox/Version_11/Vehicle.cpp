...
void init_nodes(node_info (&nodes)[MAX_NUM_OF_NODES + 1])
{
	for (int i = 0; i < MAX_NUM_OF_NODES + 1; i++)
	{
		...
		{
			nodes[i].connected_hostnames[j] = "";
			nodes[i].connected_ports[j] = "";
		}
	}
}
...
void display_all_node_data(node_info (&nodes)[MAX_NUM_OF_NODES + 1])
{
	...
	for (int i = 1; i < MAX_NUM_OF_NODES + 1; i++)
	{
		...
		for (int j = 0; j < MAX_NUM_OF_NODES; j++)
		{
			if (nodes[i].connected_hostnames[j].compare("") != 0)
			{
...
int size_of_packet_minus_hostname(tx_packet packet_in)
{
	...
	size = size + sizeof(packet_in.previous_hop_port);
...
int main(int argc, const char * argv[])
{
	...
	else
	{
		...
		packet_out.previous_hop_port = atoi(port_num.c_str()); // Convert string to int
...
//		cout << "Hostname: " << packet_out.hostname << '\n';
		...
		// Create thread to listen for incoming packets
		rc = pthread_create(&server_thread, NULL, start_receiving, &nodes[my_node_num].node_port_number);
		// check for creation errors
		if (rc)
		{
			cout << "Error: unable to create thread," << rc << endl;
...
//		nodes[my_node_num].node_port_number = port_num;
	...
	while (1)
	{
		...
		if (!Buffer.empty())
		{
			...
			// If Live we forward, if not, just update Nodes DS, then drop.
