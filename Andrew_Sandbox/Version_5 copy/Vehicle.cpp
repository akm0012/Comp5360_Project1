...
//		if (can_transmit) {
		...
		for (int i = 1; i < MAX_NUM_OF_NODES + 1; i++)
		{
			...
			// Else we are not in range of that node.
			else {
#ifdef DEBUG
...
//							nodes[my_node_num].connected_ports[j] = "N/A";
		...
		// TODO: Use the Config file to see if I need to disconnect from any nodes
	...
	//	rewrite_config_file(nodes);
		...
		if (!Buffer.empty())
		{
			...
			// CORRECTION: I think this should be done outside this if statement, otherwise
			// we may never connect to any nodes.
