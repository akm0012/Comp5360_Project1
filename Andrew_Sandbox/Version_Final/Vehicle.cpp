...
int number_of_RBA_packets_forwarded_more_than_once;
 ...
 * NOTE: Node 1 and 11 must be on different host
 */
...
/*
 * Gets the size of the packet minus the space reserved for the hostname.
 * This is used so we can send a string over a udp packet and know where to
...
/*
 * Sends a [packet_out] UDP packet to [hostname_to_send] on port [port_to_send].
 */
 ...
 * the global boolean can_transmit to true. Used for packet status sending.
 */
...
/*
 *	This starts a thread that listens for incoming packets on [port_in] and
...
//		tx_packet packet_in;
...
#ifdef DEBUG_NEW
			cout << "Received Packet Buffer Error! Incoming address == 0\n";
#endif
		...
		// Check edge case where our buffer explodes
		if (Buffer.size() > 1000000)
		{
#ifdef DEBUG_NEW
			cout << "Buffer Max Size(): " << Buffer.max_size() << '\n';
			cout << "Buffer size: " << Buffer.size() << '\n';
			cout << "UNKNOWN ERROR: BUFFER EXPLODED!!! FLUSHING!\n";
#endif
			Buffer.clear();
#ifdef DEBUG_NEW
			cout << "After Clear: Buffer size: " << Buffer.size() << '\n';
#endif
		}
 ...
 *
 * @return A boolean indicating if the packet was sent
 */
...
static void signalHandler(int signo)
{
	...
	float percent_non_admin_rebroadcasted_2plus_packets = 0;
	...
	if (total_packets_rx != 0)
	{
		...
		cout << "Number of Valid Packets Rx:\t\t\t" << number_of_valid_packets_rx << "\n";
		cout << "Number of Valid Old Packets Rx:\t\t\t" << number_of_old_packets_rx << "\n";
		cout << "Number of Valid Redundent Packets Rx:\t\t" << number_of_valid_redundent_packets_rx << "\n";
		cout << "Number of Valid Packets Rebroadcasted 2+ times:\t" << number_of_RBA_packets_forwarded_more_than_once << "\n";
		...
		cout << "Percent of Valid (processed) packets:\t\t" << percent_non_admin_packets << "%\n";
		
		percent_non_admin_old_packets = ((float)number_of_old_packets_rx / (float)number_of_valid_packets_rx) * 100;
		cout << "Percent of Valid OLD packets:\t\t\t" << percent_non_admin_old_packets << "%\n";
		
		percent_non_admin_redundent_packets = ((float)number_of_valid_redundent_packets_rx / (float)number_of_valid_packets_rx) * 100;
		cout << "Percent of Valid duplicate RBA packets:\t\t" << percent_non_admin_redundent_packets << "%\n";
		
		percent_non_admin_rebroadcasted_2plus_packets = ((float)number_of_RBA_packets_forwarded_more_than_once
														 / (float)number_of_valid_packets_rx) * 100;
		cout << "Percent of Valid Packets Re-Tx 2+ times:\t" << percent_non_admin_rebroadcasted_2plus_packets << "%\n";
	}
	else
	{
...
int main(int argc, const char * argv[])
{
	...
	total_packets_rx = 0;
	number_of_RBA_packets_forwarded_more_than_once = 0;
...
//						// -- DEBUG CODE --
	...
	else
	{
		...
		// Only write to config file so other nodes can enter highway
...
//	int micro_s = 100000; // Set the timer to 100 milliseconds
	...
	while (1)
	{
		...
		if (!Buffer.empty())
		{
			// NOTE: We manage adding new nodes in the receiver listener
			total_packets_rx++;
					...
					if (my_cache.highest_sequence_num[cache_index] <= packet_in.sequence_num)
					{
						...
						else if (my_cache.highest_sequence_num[cache_index] == packet_in.sequence_num)
						{
							...
							if (shall_I_forward)
							{
								...
								// Keep track of the number of packets forwarded more than once
								number_of_RBA_packets_forwarded_more_than_once++;
							}
						}
						...
						if (shall_I_forward)
						{
							...
							for (int y = 1; y < MAX_NUM_OF_NODES + 1; y++)
							{
								...
								// Check to make sure we don't send back to the original source
								else if (packet_copy.node_number_source == y)
								{
#ifdef TRACE
									cout << "RBA: Do not send to node: " << y << " this is the packet's source.\n";
#endif
								}
...
//			cout << "\tSpeed: " << speed_meter_per_sec << '\n';
			...
			if (my_node_num == 1 && Buffer.size() == 0)
			{
